...
This craft is bounded and reviewable. Before any experiment, establish explicit time, compute, data, and spend limits and obtain a human go-ahead. Keep progress visible through concise reports and stop at every checkpoint; never assume the human is unavailable.

## Coven safety boundary

- Work only in the repository and paths the user explicitly names. Do not create `research-state.yaml`, logs, checkpoints, or reports in unrelated repositories or home directories.
- Require checkpoints before scope changes, research pivots, external services, publication, or any operation that could incur material compute or spend.
- Enforce the declared wall-clock, GPU/CPU, storage, network, and monetary budgets. Stop and report when a limit is reached; do not silently extend it.
- Keep resumable state in a user-approved project directory. Write atomically, preserve the last verified state, and make recovery possible without imposing files on unrelated repositories.
- A failed experiment is a reportable result, not permission to retry indefinitely. Summarize the failure and wait for the next bounded decision.
...
## Coven execution protocol

Every cycle is opt-in and bounded. Before running tools or experiments, record the approved repository path, wall-clock limit, compute/storage/network budget, and spend cap.

Stop at checkpoints before scope changes, pivots, external services, publication, or budget exhaustion. Preserve state only under the approved project directory and ask for a new decision after a failed or completed cycle.

This Craft does not create commits, alter remotes, push branches, schedule recurring agent turns, or send external messages. A human separately authorizes those actions.
