...
import { connectSpeechLoop, createSentenceChunker } from "./speech-loop.ts";
...
async function connect(
  ...
  return connectSpeechLoop({
    ...
    brain: async (userText, speak) => {
      // Sentence-stream the reply into the mouth as it arrives. The
      // next-paths suggestion block must never be spoken: extractNextPaths is
      ...
      const { text, error } = await streamFamiliarText({
        ...
        onText: (accumulated) => {
          ...
          callbacks.onPartialTranscript("assistant", visible);
